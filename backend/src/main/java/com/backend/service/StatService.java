...
public class StatService implements StatServiceFacade {
  ...
  /**
   * Returns the list of CurrencyDto objects, representing the page feed
   * of currency conversions.
   * @return list of CurrencyDto objects containing the following fields:
   * dateTime, curFrom, curTo, sumBeforeConversion, exRate, sumAfterConversion
   */
  public List<CurrencyDto> getHistoryPageFeed(Long offset, Long limit){
    return statRepository.findAllAndOffsetAndLimit(offset, limit)
      .stream()
      .map(this::convertStatToCurrencyDto)
      .collect(Collectors.toList());
  }
